...
The other major type of graph that you may wish to work with is a directed graph. In `igraph` the direction of an edge is specified using the `+` sign. For example, specifying `A-+ B` would yield $A\rightarrow B$, specifying `A +- B` would yield $A \leftarrow B$, and `A +-+ B` gives $A \doublearrow B$. 
