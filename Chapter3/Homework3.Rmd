...
def objTwo(k, data, dist=chebyshev, sign = 1):
  W_hat = k * data.l * np.power(data.g,2) #note data should be a pandas data frame with columns named l and W
  return(sign*dist(data.W, W_hat))
...
for i in critList:
  ...
  for j, k in dataList.items():
    ...
    results = results.append({'Criteria': i.__name__, 'data': j, 'par': newResult.x[0], 'value': newResult.fun, 
                                'convergence': newResult.status },ignore_index=True)
...
for i in critList:
  ...
  for j, k in dataList.items():
    ...
    results2 = results2.append({'Criteria': i.__name__, 'data': j, 'par': newResult.x[0], 'value': newResult.fun, 
                                'convergence': newResult.status },ignore_index=True)
...
for i in critList:
  for j,k in dataList.items():
    ...
    plotData = plotData.append(pd.DataFrame({'l':lVals, 'Criteria': i.__name__, 'Data': j, 'W': modelOne(lVals,p1), 'Model': 'One' }))
    plotData = plotData.append(pd.DataFrame({'l':lVals, 'Criteria': i.__name__, 'Data': j, 'W': modelTwo(lVals,gVals,p2), 'Model': 'Two'}))
