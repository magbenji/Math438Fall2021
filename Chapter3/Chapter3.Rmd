...
Verify that Python is giving the same answers:

```{python Truth Py}

y = np.asarray(r.y)
yhat = np.asarray(r.yhat)
...
If we want to get the same type of results in Python as we did in R, we can use the `SciPy` function `minimize`. The syntax and methods are very similar. 

```{python ObjectiveFunction Py}
#note the addition of the "sign" variable here to switch to maximization in Python
def objectiveFn(x, xvals, yvals, dist=chebyshev, sign = 1):
  yhat = x[1]*xvals + x[0]
  return(sign*dist(yvals, yhat))
  
guess = np.array([r.intercept, r.slope])
...
Let's try the same exercise using Python's `minimize()` routine:

```{python Minimize}
import scipy.optimize as optim

ChebyFit = optim.minimize(objectiveFn, guess, args=(x,y), method = 'Nelder-Mead')
MeanADFit = optim.minimize(objectiveFn, guess, args=(x,y,meanAD), method = 'Nelder-Mead')
LSQFit = optim.minimize(objectiveFn, guess, args=(x,y,leastSq), method = 'Nelder-Mead')
MedianADFit = optim.minimize(objectiveFn, guess, args=(x,y,medianAD), method = 'Nelder-Mead')

#note the use of the -1 being passed to the "sign" argument to switch to maximization
LLFit = optim.minimize(objectiveFn, guess, args=(x,y,llnorm,-1), method = 'Nelder-Mead')

###show estimates of R vs Python
print('Chebyshev in R:', r.ChebyFit['par'])
print('Chebyshev in Python:', ChebyFit.x)

print('Mean Absolute Deviation in R:', r.MeanADFit['par'])
print('Mean Absolute Deviation in Python:', MeanADFit.x)

print('Least Squares in R:', r.LSQFit['par'])
print('Least Squares in Python:', LSQFit.x)

print('Median Absolute Deviation in R:', r.MedianADFit['par'])
print('Median Absolute Deviation in Python:', MedianADFit.x)

print('Maximum Likelihood in R:', r.LLFit['par'])
print('Maximum Likelihood in Python:', LLFit.x)
