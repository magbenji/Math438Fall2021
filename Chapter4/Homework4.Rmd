...
rng = np.random
rng.seed(6851)
randx = rng.uniform(min(x), max(x), 15)
xset = np.array([])
for i in randx: xset = np.append(xset, np.argmin(np.abs(i - x)))

xset.sort()

sub_x = x[xset.astype('int')]
sub_y = y[xset.astype('int')]

plt.plot(sub_x,sub_y)
plt.xlabel('x')
plt.ylabel('y')
plt.title('Subset of 15 Points')
plt.show()
plt.close()
...
lv <- lagrange(seq(min(sub.x),max(sub.x),length = 200), sub.x, sub.y)
plot(lv, type="l", xlab="x", ylab="y", main = "High Order Approximation", lwd = 2, col="blue", ylim = plotRange(sub.y))
...
lg_data = lagrange(np.linspace(np.min(sub_x),np.max(sub_x),200), sub_x, sub_y)

plt.plot(lg_data[:,0], lg_data[:,1])
plt.scatter(sub_x, sub_y, color = 'red')
plt.ylim((0,25))
plt.xlabel('x')
plt.ylabel('y')
...
```{python}
first = pd.Series(sub_y).diff()/pd.Series(sub_x).diff()
second = first.diff()/pd.Series(sub_x).diff(periods=2)
third = second.diff()/pd.Series(sub_x).diff(periods=3)
fourth = third.diff()/pd.Series(sub_x).diff(periods=4)
fifth = fourth.diff()/pd.Series(sub_x).diff(periods=5)

div_diff = pd.DataFrame(data={'first': first, 'second': second,'third': third, 'fourth':fourth,
                    'fifth': fifth})
div_diff.plot(subplots=True, layout = (3,2))
...
lv2 <- lagrange(seq(min(sub.x2),max(sub.x2),length = 200), sub.x2, sub.y2)
lv3 <- lagrange(seq(min(sub.x3),max(sub.x3),length = 200), sub.x3, sub.y3)
plot(lv2, type="l", xlab="x", ylab="y", main = "High Order Approximation - Second Set", lwd = 2, col="blue", ylim = plotRange(sub.y2))
points(sub.x2,sub.y2, pch = 20, cex =2 , col="red")
plot(lv3, type="l", xlab="x", ylab="y", main = "High Order Approximation - Third Set", lwd = 2, col="blue", ylim = plotRange(sub.y3))
...
def objectiveFn(x, xvals, yvals):
  A, B = x
  lm = smf.glm('Y ~ np.sin(A*X + B)', data = pd.DataFrame({'X': xvals, 'Y':yvals}) )
  lmf = lm.fit()
  return lmf.aic

guess = [1, 0]

fit = optim.minimize(objectiveFn, guess, (x,y), method = 'Nelder-Mead')
fit.x
```
...
```{python}
final = smf.glm('Y ~ np.sin(fit.x[0]*X + fit.x[1])',  data = pd.DataFrame({'X': x, 'Y':y}))
final_fit = final.fit()
final_fit.summary()
```
...
```{python}
plt.scatter(x,y, facecolors='none', edgecolors='k', alpha = 0.25)
plt.plot(np.linspace(np.min(x), np.max(x), 200), final_fit.predict(pd.DataFrame({'X':np.linspace(np.min(x), np.max(x), 200)}), transform=True), color = 'r')
