...
Let's run the same code using Python. To get the `ecdf()` function we use the `statsmodels` library.
...
rng = np.random
vals = np.concatenate((rng.normal(0,2,100),rng.normal(10,3,100),rng.normal(20,5,100)))

plt.hist(vals, density = True,  bins = 30, color = 'r')
plt.show()
plt.close()

CDF = ed.ECDF(vals)
InvCDF  = ed.monotone_fn_inverter(CDF,np.linspace(np.min(vals) - 1, np.max(vals) + 1,1000)) #find the inverse CDF

vals2 = InvCDF(rng.uniform(size=2000))

plt.hist([vals,vals2], density = True, bins = 30, color = ['red','blue'], label = ['Mixture Distribution','eCDF'])
plt.legend()
...
Of course we can basically do the same thing with Python. Here we use the function `brentq` from `scipy.optimize` to find a root.

```{python brentq}
import scipy.optimize as optim

def inverse(y):
  return optim.brentq(lambda x: 1/(6*np.pi)*(-np.cos(x) + 1 + 3*x) - y, 0, 2*np.pi)

inverse(0.75)

qVals = np.arange(0,1.01,0.01)

plt.plot(qVals,np.vectorize(inverse)(qVals))
plt.title('Numerical Inverse of CDF')
plt.ylabel('x')
plt.xlabel('quantile')
plt.show()
plt.close()
...
## Sampling From Finite Sets with `sample()` in R or `choice()` in Python
...
Let's replicate the above using Python:

```{python choice}

elections = 100
winner = rng.choice(['Bob','Tom','Jane'], elections, replace = True, p = [5,1,8]/np.sum([5,1,8])) #note you have to give python an actual vector that sums to 1 for p
pd.Series(winner).value_counts()/elections

NSamples = 30
winner = rng.choice(['Bob','Tom','Jane'], (elections, NSamples), replace = True, p = [5,1,8]/np.sum([5,1,8]))

Winner = pd.DataFrame(winner)
Winner.apply(lambda x: pd.Series(x).value_counts()/100).mean(axis = 1)

np.var(Winner.apply(lambda x: pd.Series(x).value_counts()/100).transpose())**0.5

rng.seed(95839)
x = rng.normal(size=50)
x = np.concatenate((x ,rng.normal(5,1,3))) #add in some outliers
np.mean(x) #not near 0!
np.var(x)**0.5 #not near 1!

import scipy.special as scispec

scispec.comb(x.size, x.size//4, exact=True) #integer division is done with "//" in Python

NResamp = 1000
resample = rng.choice(x,(x.size//4,NResamp)) #matrix with dimensions x.size//4 x NResamp

plt.hist(resample.mean(axis=0))
plt.title('Resampled Means')
plt.xlabel('Mean')
plt.show()
plt.close()

np.quantile(resample.mean(axis=0),[0.025,0.5,0.975]) #95% interval and median

plt.hist(resample.var(axis=0)**0.5)
plt.title('Resampled Error')
plt.xlabel('Error')
plt.show()
plt.close()

np.quantile(resample.var(axis=0)**0.5,[0.025,0.5,0.975]) #95% interval and median

##### with replacement

resample = rng.choice(x,(x.size,NResamp), replace = True) #matrix with dimensions x.size//4 x NResamp

plt.hist(resample.mean(axis=0))
plt.title('Resampled Means (with Replacement)')
plt.xlabel('Mean')
plt.show()
plt.close()
...
plt.hist(resample.var(axis=0)**0.5)
plt.title('Resampled Error (with Replacement)')
plt.xlabel('Error')
plt.show()
plt.close()
...
```{python}
x = np.exp(-1*rng.exponential(3,10000)) * np.sin(rng.normal(2,0.5,10000))
plt.hist(x, bins = 100)
plt.show()
plt.close()
