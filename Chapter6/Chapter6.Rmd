...
The last thing we will investigate with Markov chains is their use in searching parameter space. To search parameter space, we initialize the chain to initial state $\Theta_0$. We then will propose a new state based on a **proposal density**. Commonly, we might propose new values of the chain based on a multivariate normal distribution, however, other proposals kernels might be necessary depending on the problem. Furthermore we might need to constrain the proposal space if we know there are particular constraints on the parameters (e.g., $\gt0$). Once we propose a new state, we need to use an **acceptance algorithm**. A very common of these is the Metropolis-Hastings algorithm. The algorithm to update the chain works as follows:
  ...
  4. Generate a random uniform $u$. If $u \gt \alpha$, reject the proposal. Append the "winning" state to the chain.
...
y = np.random.normal(3*x+2,3,100)

plt.scatter(x,y)

chain = {"slope":0,"intercept":0} #initialize chain

Chain = pd.DataFrame(chain, index = [0])

def proposal(theta, sd = 0.1): return np.random.normal(theta, sd, theta.size)
def ll_theta(theta, x, y): return np.sum(stats.norm.logpdf(y, x*theta[0] + theta[1]))

nSteps = 1000

for i in range(nSteps):
  old = Chain.iloc[i].to_numpy()
  new = proposal(old)
  log_alpha = ll_theta(new, x, y) - ll_theta(old, x, y)
  next_prop = old if np.log(np.random.uniform()) > log_alpha else new 
  Chain = Chain.append(pd.DataFrame(next_prop.reshape(1,2), columns = ['slope','intercept']), ignore_index = True)
    
Chain.plot(subplots = True)
plt.xlabel('Iteration')
plt.show()
plt.close()

###still see significant autocorrelation for the intercept, iterate more
for i in 1000 + np.arange(1000):
  old = Chain.iloc[i].to_numpy()
  new = proposal(old)
  log_alpha = ll_theta(new, x, y) - ll_theta(old, x, y)
  next_prop = old if np.log(np.random.uniform()) > log_alpha else new 
...
```{python}
FinalChain = Chain.iloc[np.arange(1001,2001, 10)] #take every 10th sample in the last 1000 iterations

FinalChain.plot(subplots = True)
plt.show()
plt.close('all')

plt.hist(FinalChain.slope, bins = 20)
plt.show()
plt.close()

plt.hist(FinalChain.intercept, bins = 20)
plt.show()
plt.close()

np.mean(FinalChain, axis = 0)
np.median(FinalChain, axis = 0)

np.quantile(FinalChain.slope, [0.025,0.975]) #95% CI of slope
np.quantile(FinalChain.intercept, [0.025,0.975]) #95% CI of intercept

import statsmodels.formula.api as sfa
sfa.ols('y ~ x', pd.DataFrame({'x':x, 'y':y})).fit().summary()
